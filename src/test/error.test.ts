...
import { hasErrors, throwErrors } from "../main/error.ts";

describe("error", () => {
    describe("hasErrors", () => {
        function expectNonEmptyErrors(errors: readonly [unknown, ...unknown[]]): void {
            void errors;
        }

        it("detects whether collected failures exist", () => {
            assertSame(hasErrors([]), false);
            assertSame(hasErrors([ "boom" ]), true);
        });

        it("narrows collected failures to a non-empty tuple", () => {
            const errors: unknown[] = [ "boom" ];

            // @ts-expect-error Must not compile because a plain array is not known to be non-empty yet.
            expectNonEmptyErrors(errors);

            if (hasErrors(errors)) {
                expectNonEmptyErrors(errors);
                assertSame(errors[0], "boom");
            }
        });
    });

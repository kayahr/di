...
export class Injector {
    ...
    /**
     * Registers a token-qualified factory with no parameters.
     *
     * @param token   - The sole qualifier under which the factory is registered.
     * @param factory - The factory function to register.
     * @param options - Optional registration options.
     *
     * @template Value - The created dependency type.
     */
    public setFactory<Value>(token: InjectionToken<Value>, factory: Factory<Value, []>,
        options?: Omit<InjectableOptions<[], Value>, "provide" | "token">): this;

    /**
     * Registers a token-qualified factory with fully injected parameters.
     *
     * @param token   - The sole qualifier under which the factory is registered.
     * @param factory - The factory function to register.
     * @param options - Registration options including the injected factory parameters.
     ...
     * @template Params - The factory parameter types.
     */
    public setFactory<Value, Params extends unknown[]>(token: InjectionToken<Value>, factory: Factory<Value, Params>,
        options: Omit<InjectableOptions<Qualifiers<Params>, Value>, "provide" | "token"> & { inject: Qualifiers<Params> }): this;

    /**
     * Registers a transient token-qualified factory whose parameters may use `null` pass-through placeholders.
     *
     * @param token   - The sole qualifier under which the factory is registered.
     * @param factory - The factory function to register.
     * @param options - Registration options including the injected factory parameters and transient lifetime.
     ...
     */
    public setFactory<Value, Params extends unknown[]>(token: InjectionToken<Value>, factory: Factory<Value, Params>,
        options: Omit<InjectableOptions<NullableQualifiers<Params>, Value>, "provide" | "token">
        & { inject: NullableQualifiers<Params>, lifetime: typeof Lifetime.TRANSIENT }): this;

    /**
     * Registers the given injectable factory in the target scope.
     *
     * @param qualifier - The class or sole injection token under which the factory is registered.
     * @param factory   - The factory function which creates the value (synchronous or asynchronous).
     * @param options   - Options for the injectable. Optional if factory has no parameters. Class-based registrations can use `provide` to register
     *                    additional base-type qualifiers.
     *
     * @template Value  - The created dependency type.
     * @template Params - The factory parameter types.
     */
    public setFactory<Value, Params extends unknown[]>(qualifier: Class<Value> | InjectionToken<Value>, factory: Factory<Value, Params>,
            { scope, inject, lifetime = Lifetime.SINGLETON, token, provide }: InjectableOptions<NullableQualifiers<Params>, Value> = {}): this {
        const tokenQualified = typeof qualifier !== "function";
        const type = tokenQualified ? factory : qualifier;
        this.#getRegistry(scope).register(new Injectable({
            type,
            factory: args => tokenQualified ? factory(...args as Params) : factory.apply(type, args as Params),
            params: inject,
            token: tokenQualified ? qualifier : token,
            provide: tokenQualified ? [] : this.#normalizeProvidedTypes(type, provide),
            registerSelf: !tokenQualified,
            lifetime
        }));
        return this;
    }
